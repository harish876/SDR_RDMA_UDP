...
enum class ControlMsgType : uint8_t {
    ...
    ACK = 4,            // ADD THIS - Selective repeat ACK with bitmap
    COMPLETE_ACK = 5,   // CHANGED from 4 to 5 - Receiver acknowledges transfer completion
    INCOMPLETE_NACK = 6 // CHANGED from 5 to 6 - Receiver indicates transfer incomplete
};
...
struct ControlMessage {
    ...
    // ADD THIS - Bitmap for selective repeat ACKs
    static constexpr size_t MAX_BITMAP_BYTES = 4096; // supports up to 32768 packets
    uint8_t bitmap[MAX_BITMAP_BYTES];
    uint32_t bitmap_size_bytes;      // actual size used
