...
struct SDRSendHandle {
    ...
    // ADD THESE - Selective Repeat fields
    size_t window_size = 16;           // SR window size
    std::vector<bool> acked_packets;   // Track which packets are ACKed
    uint32_t base_packet = 0;          // Oldest un-ACKed packet
    uint32_t next_packet = 0;          // Next packet to send
};
...
struct SDRStreamHandle {
    uint32_t msg_id;
    uint32_t generation;  // <-- Add this line
    std::shared_ptr<ConnectionContext> connection_ctx;
    const void* user_buffer;
    size_t buffer_size;
