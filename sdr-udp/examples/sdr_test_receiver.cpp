...
int main(int argc, char* argv[]) {
    ...
    // Use configured UDP server IP if provided (needed for remote runs).
    {
        // Default to the receiver's external IP for remote runs; override via config if needed.
        std::string cfg_ip = config.get_string("udp_server_ip", "128.110.219.167");
        std::strncpy(params.udp_server_ip, cfg_ip.c_str(), sizeof(params.udp_server_ip) - 1);
