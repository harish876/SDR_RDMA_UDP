...
int sdr_recv_post(SDRConnection* conn, void* buffer, size_t length, SDRRecvHandle** handle) {
    ...
    while (true) {
        ...
        std::cerr << "[SDR API] Skipping unexpected control message type "
                  << static_cast<int>(offer.msg_type) << std::endl;
    }
    ...
    uint32_t proposed_mtu = params.mtu_bytes ? params.mtu_bytes
                                             : (offer.params.mtu_bytes ? offer.params.mtu_bytes
                                                                       : SDRPacket::MAX_PAYLOAD_SIZE);
    ...
    params.packets_per_chunk = params.packets_per_chunk
                                   ? params.packets_per_chunk
                                   : (offer.params.packets_per_chunk ? offer.params.packets_per_chunk : 64);
    params.num_channels = offer.params.num_channels
                              ? offer.params.num_channels
    ...
    *handle = recv_handle;
    ...
    while (true) {
        ...
        std::cerr << "[SDR API] Skipping unexpected control message type "
                  << static_cast<int>(accept.msg_type) << std::endl;
    }
...
int sdr_send_post(SDRConnection* conn, const void* buffer, size_t length, SDRSendHandle** handle) {
    ...
    while (true) {
        ...
        std::cerr << "[SDR API] Skipping unexpected control message type "
                  << static_cast<int>(cts_msg.msg_type) << std::endl;
    }
