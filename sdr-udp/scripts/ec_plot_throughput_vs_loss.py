...
LOSS_CSV = os.path.join(DATA_DIR, "ec_Data.csv")  # <-- change filename if needed
...
def main():
    ...
    plot_value_vs_loss(
        sndr_tp_series,
        title="UDP EC - Sender Throughput vs Loss",
        ylabel="Sender Throughput (Mbps)",
        filename="ec_sndr_throughput_vs_loss.png",
    )

    # 2) RCVR throughput vs loss
    plot_value_vs_loss(
        rcvr_tp_series,
        title="UDP EC - Receiver Throughput vs Loss",
        ylabel="Receiver Throughput (Mbps)",
        filename="ec_rcvr_throughput_vs_loss.png",
    )
    ...
    plot_value_vs_loss(
        sndr_tt_series,
        title="UDP EC - Sender Transfer Time vs Loss",
        ylabel="Sender Transfer Time (ms)",
        filename="ec_sndr_transfer_time_vs_loss.png",
    )

    # 4) RCVR transfer time vs loss
    plot_value_vs_loss(
        rcvr_tt_series,
        title="UDP EC - Receiver Transfer Time vs Loss",
        ylabel="Receiver Transfer Time (ms)",
        filename="ec_rcvr_transfer_time_vs_loss.png",
    )
