...
LOSS_CSV = os.path.join(DATA_DIR, "baseline_Data.csv")  # <-- change filename if needed
...
def main():
    ...
    plot_value_vs_loss(
        sndr_tp_series,
        title="UDP Baseline - Sender Throughput vs Loss",
        ylabel="Sender Throughput (Mbps)",
        filename="baseline_sndr_throughput_vs_loss.png",
    )

    # 2) RCVR throughput vs loss
    plot_value_vs_loss(
        rcvr_tp_series,
        title="UDP Baseline - Receiver Throughput vs Loss",
        ylabel="Receiver Throughput (Mbps)",
        filename="baseline_rcvr_throughput_vs_loss.png",
    )
    ...
    plot_value_vs_loss(
        sndr_tt_series,
        title="UDP Baseline - Sender Transfer Time vs Loss",
        ylabel="Sender Transfer Time (ms)",
        filename="baseline_sndr_transfer_time_vs_loss.png",
    )

    # 4) RCVR transfer time vs loss
    plot_value_vs_loss(
        rcvr_tt_series,
        title="UDP Baseline - Receiver Transfer Time vs Loss",
        ylabel="Receiver Transfer Time (ms)",
        filename="baseline_rcvr_transfer_time_vs_loss.png",
    )
